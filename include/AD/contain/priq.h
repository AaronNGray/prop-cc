...
#ifndef priority_queue_ordered_by_function_h
#define priority_queue_ordered_by_function_h

///////////////////////////////////////////////////////////////////////
//  Class PriQ<T> is a binary heap array based
...
class PriQ : public A
{
  int (*compare)( const T&, const T&); // comparison function
...
public:
  ...
  PriQ(int size, int (*cmp)(const T&, const T&) = 0)
      : A(size), elemCount(0), compare(cmp)
  {}
  PriQ(const PriQ& Q) : A(Q.capacity())
  {
    *this = Q;
  }
  ~PriQ()
...
template<class T, class A>
PriQ<T,A>& PriQ<T,A>::operator = (const PriQ<T,A>& Q)
{
  if (this != &Q)
  {
    compare   = Q.compare;
...
template<class T,class A>
void PriQ<T,A>::dequeue(register int i)
{
  register int j;
...
template<class T, class A>
void PriQ<T,A>::enqueue(const T& element)
{
  register int i, j;
...
template<class T, class A>
void PriQ<T,A>::requeue(register int i)
{
  register int j;
