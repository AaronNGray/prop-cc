...
#ifndef hash_table_with_ordered_hashing2_h
#define hash_table_with_ordered_hashing2_h

////////////////////////////////////////////////////////////////////////////
// Class OHashTable2 implements a hash table using an ordered hashing
...
////////////////////////////////////////////////////////////////////////////
//  Class |OHashTable2| is parameterized with the class of the
...
public:
  ...
  ////////////////////////////////////////////////////////////////////
  OHashTable2(int    initial_size   = 32,
  ...
  //  Assignment
  ////////////////////////////////////////////////////////////////////
  void operator = (const OHashTable2&);
...
template <class K, class V, class C>
inline Ix OHashTable2<K,V,C>::find_next(register int i) const
{
  while (i < table_size)
...
template <class K, class V, class C>
void OHashTable2<K,V,C>::operator = (const OHashTable2<K,V,C>& t)
{
  if (this != &t)
  {
    delete [] keys;
...
template <class K, class V, class C>
Ix OHashTable2<K,V,C>::lookup(const K& key) const
{
...
template <class K, class V, class C>
void OHashTable2<K,V,C>::resize(int new_size)
{
  ...
  for (int i = 0; i < table_size; i++)
  {
    if (status[i] == Cell_used)
    {
      ...
      for (;;)
      {
        ...
        int r = C::compare(new_keys[j],keys[i]);
        if (r > 0)
        { // exchange entries and continue
          K k = new_keys[j];
          new_keys[j] = keys[i];
