...
#ifndef linear_probing_hash_table2_h
#define linear_probing_hash_table2_h

////////////////////////////////////////////////////////////////////////////
//  Class LHashTable2 implements a hash table using a simple
...
////////////////////////////////////////////////////////////////////////////
//  Class |LHashTable2| is parameterized with the class of the
...
public:
  ...
  ////////////////////////////////////////////////////////////////////
  LHashTable2(int initial_size      = 32,
  ...
  //  Assignment
  ////////////////////////////////////////////////////////////////////
  void operator = (const LHashTable2&);
...
template <class K, class V, class C>
void LHashTable2<K,V,C>::operator = (const LHashTable2<K,V,C>& t)
{
  if (this != &t)
  {
    delete [] keys;
...
template <class K, class V, class C>
Ix LHashTable2<K,V,C>::lookup(const K& key) const
{
  int i = C::hash(key) % table_size;
  for (;;)
  {
    switch (status[i])
    {
...
template <class K, class V, class C>
Ix LHashTable2<K,V,C>::insert(const K& key, const V& value)
{
  /////////////////////////////////////////////////////////////////////
  ...
  if (elem_count >= max_load)
    resize();
  register int i = C::hash(key) % table_size;
...
template <class K, class V, class C>
void LHashTable2<K,V,C>::resize(int new_size)
{
  ...
  for (int i = 0; i < table_size; i++)
  {
    if (status[i] == Cell_used)
    {
      register int j = C::hash(keys[i]) % new_size;
      for (;;)
      {
