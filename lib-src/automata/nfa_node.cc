...
void NFA_Accept::closure( FastBitSet* set)
{
  set->add( state);
}

void NFA_Delta::closure( FastBitSet* set)
{
  set->add( state);
}

void NFA_Epsilon::closure( FastBitSet* set)
{
  out->closure( set);
}

void NFA_Or::closure( FastBitSet* set)
{
  out->closure( set);
  out2->closure( set);
}
...
(MemPool& mem, size_t N, size_t /*rules*/, NFA_Delta * /*A*/[])
{
  if (closure_set)
    return closure_set;
  closure_set = new (mem) FastBitSet(mem,N);
  closure_set->add
  (state);            // add accept state
  return closure_set;
}
...
(MemPool& mem, size_t N, size_t rules, NFA_Delta * A[])
{
  ...
  closure_set->add
  (state);
  out->closure(delta_set = new(mem) FastBitSet(mem,N));
